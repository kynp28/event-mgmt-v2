...
export class OrganizerRequestController {
  ...
  createRequest = async (req: Request, res: Response) => {
    ...
    res.status(201).json({ success: true, message: 'ส่งคำขอเป็นผู้จัดงานสำเร็จ', data: result });
  };
  ...
  reviewRequest = async (req: Request, res: Response) => {
    ...
    res.status(200).json({ success: true, message: `เปลี่ยนสถานะคำขอเป็น ${status} สำเร็จ`, data: result });
  };
}
